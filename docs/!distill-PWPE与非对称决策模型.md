...
> **类型：`!distill-`**（2026-08-29 定，定义见 [README §2.4](../README.md)）——我自己蒸出来的那一版。厚版原件是 [moc-@认知链路-v2](archive/moc-@认知链路-v2.md)（76k，已归档），要查论据或那 6 条锚点图产出去那里翻。

## 动机

本篇笔记要解决的问题是 (why): INTJ 在面对信息过载与不确定性时，如何进行理性决策？

本文结构 (how): 从感知层、认知层、决策层三个层面出发，以**PWPE与边际收益的对应关系**作为主干，寻找贝叶斯思维、多巴胺、认知模型以及荣格性格分析之间的关系
...
>
> *注：为了保持完整性保留了主动行动，但它不在本篇的 scope 当中。具体如何行动参见 [!distill-Ni-Te-audit](!distill-Ni-Te-audit.md) 以及 [PWPE与非对称决策模型-行动层pending](PWPE与非对称决策模型-行动层pending.md)
...
*注意，这里 items 上的 [+] 与 [-] 只是一个示例，真正做决策时，每个 items 对应是正影响还是负影响，取决于具体的 use case，不能一概而论*
...
```mermaid
flowchart TD
    A[PWPE=误差+精度] --> B[多巴胺处理 PWPE] --> C[多巴胺可以通过 Ni 直觉感知] --> D1[5x非对称模型]
    C--> D2[3x操作规则]
    D1 & D2 --> E[边际效用清单]
```
...
把「行动层」整章剥出，其他三层单独成文，完善了结论

2026-08-02

归档 `moc-@决策杠杆` 时，把它 §1.3「区别与边界」表抢救到这里——它比上面 §决策层的边际收益清单多了「典型问题 / 风险」这个「何时用哪个」的实用角度，留作参考。（决策杠杆原来的中心论点「这 5 个都是一个杠杆家族」，已被本篇「非对称模型（引擎）+ 底层逻辑（操作系统）」的更细切法取代。）
