...
> **归档说明（2026-08-29）**
>
> 本篇是厚版原件，内容已被 [PWPE与非对称决策模型](../PWPE与非对称决策模型.md)（感知 / 认知 / 决策三层）读薄吸收，移入 `docs/archive/`——只移不删，仍可引用。
>
> **它还是这几样东西的唯一出处**，不要当废稿：
>
...
> **✅ 已结案（2026-08-29 audit）**：实际走的路径是 B 的一个变体——不是在 v2 内部压缩，是另起一篇 [PWPE与非对称决策模型](../PWPE与非对称决策模型.md) 把 v2 读薄（2026-07-26 定稿），厚版 v2 整篇归档。A 方案（重命名为 hypo + 重建瘦身 moc）没走，因为 PWPE 本身就是那个「瘦身版本」，再留一张索引型 moc 是多一层；C 方案（认下「论证型 moc」这个新类型）被推翻，因为读薄之后留在 docs 里的是 PWPE 而不是 v2，要命名的是薄版那一类，不是厚版。
>
