...
> **归档说明（2026-08-29）**
>
> 本篇是厚版原件，内容已被 [PWPE与非对称决策模型](../!distill-PWPE与非对称决策模型.md)（感知 / 认知 / 决策三层）读薄吸收，移入 `docs/archive/`——只移不删，仍可引用。
>
> **它还是这几样东西的唯一出处**，不要当废稿：
>
> - canvas 锚点图那 6 条新产出（`_todo` §2.2b C007「向上到基因层、向下到外环境层与社会层如何延展」）
> - §4 怀疑机制（[!q-MVP 验证 vs 可得性启发的边界](../!q-MVP%20验证%20vs%20可得性启发的边界.md) 明确依赖 §4.3 自欺机制）
> - §1 进化层级框架的 4 层 × 3 维度（[card-@进化层级模型](../card-@进化层级模型.md) 的源头）
>
> **两处内部指令已失效**：§6.1「保留 v1，不替换不归档」——v1 已于同日一并归档；§6.4 Post-v2 决策点（A/B/C）——已由 2026-08-29 audit 结案，见该节标注。
...
§1 是**4 层框架**（基因 / 大脑 / 外环境 / 社会）；§2 §3 §4 不是平行的三个主题，而是对**同一个对象（决策机制）的三种动作** —— 理解 / 使用 / 怀疑；§5 是把大脑层的概念反向映射回基因 / 外环境 / 社会，让框架真正闭环。
...
---
graph TB
    S1["§1 进化层级框架<br/>（4 层布局）<br/>基因 / 大脑 / 外环境 / 社会"]

    subgraph BRAIN["『大脑 / 个体』层 — §2-§4 都在这层"]
        M(("决策机制<br/>同一对象"))
    ...
    S5["§5 跨层等价 (v2 真正产出)<br/>把 §2 §3 的概念反向映射回<br/>基因 / 外环境 / 社会"]
...
**关系一句话**：§1 摆好 4 层框架 → §2 装机制 → §3 用机制 → §4 审视机制 → §5 让机制映射到各层。
...
> **§1 是根**：§2、§4、§5 都从它取根据（§4 尤其明显——损失厌恶那些偏差的进化根源都在 §1）；§1 自己不接收别的章。跨章的具体连接见各子节末尾的桥梁注。
...
> **§2 五步脉络**：贝叶斯基础（§2.1）→ 补上“精度”这第三个量（§2.2）→ 精度 × 误差 = 2×2 判定这个核心（§2.3）→ 由此分出内化（§2.4）和三型故障（§2.5）。对外：§2.3 的 D 路径是 §3 的入口，§2.5 的三型故障给 §4 当机制根基。
...
> **§3 五步脉络**：先定“做什么”（§3.1）→ 再定“怎么做”（§3.2）→ 其中只有刻意练习的 3F 和 §2 贝叶斯一一对应，单拎成 §3.3 作为通往 §2 的桥 → 3F 的微观版是行动三阶段复盘（§3.4）→ 君主论把这些工具打包（§3.5）；§3.5 已碰到“何时不该信机制”，自然过渡到 §4。
...
> **§4 怀疑逐层升级**：识别哪种故障（§4.1）→ 处置（§4.2）→ 处置也可能错、需元工具（§4.3）→ 工具对了机制未必成立、升到机制层怀疑（§4.4）→ 怀疑要有边界、不能无限退步、最终回到行动（§4.5）。对外：§4.1 接 §2.5 三型故障 + §1 偏差根源；处置动作复用 §3 的 toolkit；§3 用得越顺，越催生 §4 的必要。
...
> **§5 是 §1.5 同构嵌套的具体化**：6 条跨层等价，各把一个大脑层概念接回对应处——自由能 → §1.1+§2.2、多巴胺 → §2.2、hardwire/livewire → §1.5、心理表征 → §2.4、3F → §3.3、二八/ESS → §1.4+§3.1。
>
> ⚠️ **§3.3 retrofit（待办）**：§3.3 当前 mapping 缺精度，§5.4 升级后 §3.3 需补一句指向 §5.4 升级版。
...
> **⚠️ 本节已失效（2026-08-29 audit）**：v1 与本篇同日一并归档。当时定「不归档」的四条理由里，前两条（audit trail / v1 是 v2 的 setup）在「只移不删、archive 仍可引用」的做法下仍然成立，所以归档不违背它们；第三条（选 A 方案则 v1 留作索引版本）已随 §6.4 结案作废。归档前做的事：v1 唯一没被 PWPE / Ni-Te-audit 接住的一块——奥卡姆剃刀的机制解释——已择出成 [card-@奥卡姆剃刀](../card-@奥卡姆剃刀.md)；三张 keystone card 指向 v1 §3 / §4c 的脚注已改指 PWPE 与 Ni-Te-audit。
...
> **✅ 已结案（2026-08-29 audit）**：实际走的路径是 B 的一个变体——不是在 v2 内部压缩，是另起一篇 [PWPE与非对称决策模型](../!distill-PWPE与非对称决策模型.md) 把 v2 读薄（2026-07-26 定稿），厚版 v2 整篇归档。A 方案（重命名为 hypo + 重建瘦身 moc）没走，因为 PWPE 本身就是那个「瘦身版本」，再留一张索引型 moc 是多一层；C 方案（认下「论证型 moc」这个新类型）被推翻，因为读薄之后留在 docs 里的是 PWPE 而不是 v2，要命名的是薄版那一类，不是厚版。
>
> 类型命名（`type: 待定` 这件事）同日也定了：样本是「厚版 / 薄版」成对出现的两对，n=2 成立，薄版那一类定名 `!distill-`（见 [README §2.4](../../README.md)）。本篇的薄版是 [!distill-PWPE与非对称决策模型](../!distill-PWPE与非对称决策模型.md)。经过记在 [archive/_done.md](_done.md)（原挂 `_todo` 信号 4 · B1，该信号已一并归档）。
