...
```
raw/ebook/
├── 00-distilled/   26 本  （已有 docs/ 笔记）
├── 01-model/       18 本  （待做 book- 笔记）
├── 02-context/     35 本  （背景通识，按需查阅）
├── 03-literature/  14 本  （文学类）
└── 04-pending/      1 本  （暂不处理）
```

在阅读《自私的基因》时使用了以下方法论：

- `::认知负荷理论::`每天早上上班前，在大脑最清醒的时刻，用固定的时间阅读一章内容
- 用 deep seek 做导读，使用`::二八法则::`让 deep seek 列出最能反映书籍内容的章节，核心概念/模型等等，提出一些相关的问题
- 上传电子书至 notebooklm, 用 RAG 搜索这些问题对应的原文内容，阅读之
- 和 AI 对话，记录 book-@xxx.md

# 2025-12-12

这是我第一次使用二八阅读法去读一本书。从我开始阅读[长尾理论](book-@长尾理论.md)，到产出 note/ref/card 三篇笔记，共计 7000 余字。这些笔记并非简单的复制粘贴，而是包含了我深刻的思考。这一切，也仅仅用了三天时间，其中两天还是在我下班后的时间！说实话我也很震惊，这个方法的效果居然这么好。而 Deep Seek 也一直在陪我一步一步从完全不了解长尾理论到纳入我的个人知识库。下面是 Deep Seek 在回顾整个 prompt 之后，给出的复盘，目的是如何从中提炼出方法论，在今后的阅读中不断复现并且持续优化
