...
# park：!DRAFT 行动层（暂存，未验证）

这是 [!DRAFT](!DRAFT.md) 原来的第六章「行动层」，2026-07-25 从 !DRAFT 剥出来放这。

**为什么剥出来 park**：

- !DRAFT 前五章讲的是「Ni 理论」（世界怎么运作），行动层讲的是「在做的行动怎么追踪」。中间隔着 [书接上回](书接上回-能力圈勘定.md) 那一步——先定「什么行动能做 / 已经在做」（to do），才轮到「在做的怎么追踪」（doing）。所以行动层不该焊在 !DRAFT 里，逻辑顺序是：!DRAFT（Ni 理论）→ 书接上回（能力圈：能做/在做）→ 行动层（在做的怎么追踪）。
...
| 把直觉转成陈述性行动假设（「如果我做 X，预测发生 Y」）+ 事后校准直觉精度 | **待验证**——这是 !DRAFT 的真发现（「直觉也能 tracking」，见 [tracking-@PKM](tracking-@PKM.md) 2026-07-19 产出④），但作者还没跑过任何一次 | 记下来、标「待验证」，别当已确立的规则 |
