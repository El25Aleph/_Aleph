...
- PWPE与非对称决策模型 前五章讲的是「Ni 理论」（世界怎么运作），行动层讲的是「在做的行动怎么追踪」。中间隔着 [书接上回](书接上回-能力圈勘定.md) 那一步——先定「什么行动能做 / 已经在做」（to do），才轮到「在做的怎么追踪」（doing）。所以行动层不该焊在它里，逻辑顺序是：PWPE与非对称决策模型（Ni 理论）→ 书接上回（能力圈：能做/在做）→ 行动层（在做的怎么追踪）。
