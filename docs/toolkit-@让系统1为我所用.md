...
- **[PWPE与非对称决策模型](PWPE与非对称决策模型.md) + [Ni-Te-audit](Ni-Te-audit.md)**：底层认知机制（贝叶斯 → 自由能 → 系统 1/2 → 多巴胺 → 刻意练习）。本 toolkit 是**上层应用**——把那条链路的最后一环（"如何用刻意练习改造系统 1"）展开为可执行的 5 策略。（原写的是 `toolkit-@认知链路`，全库查无此文件，2026-08-29 audit 修正为实际出处。）
